...
impl AddrSpace {
    ...
    const DATE_BYTES: usize = 4 * 1024 * 1024 * 1024;

    const DATA_PAGES: usize = Self::DATE_BYTES / Self::PAGE_BYTES;

    const PD_PAGES: usize = bytes_to_pages(Self::DATA_PAGES * size_of::<PageDescriptor>());

    const BITMAP_PAGES: usize = bytes_to_pages(Self::DATA_PAGES * size_of::<Bitmap>());
    ...
    pub(crate) fn new_reserved(&self) -> PdList {
        debug_assert_eq!(
            ...
            self.used_pds.get(),
            "You can't call new_reserved after init"
        );
...
impl<'space> PdRef<'space> {
    ...
    fn pd(self) -> &'space PageDescriptor {
        unsafe {
            let offset = self.idx.0 as usize * size_of::<PageDescriptor>();
    ...
    fn bitmap(self) -> &'space Bitmap {
        unsafe {
            let offset = self.idx.0 as usize * size_of::<Bitmap>() + AddrSpace::PD_BYTES;
